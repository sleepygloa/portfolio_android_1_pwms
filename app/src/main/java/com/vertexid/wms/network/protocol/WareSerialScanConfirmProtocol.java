...
/**
 * 입고 시리얼 스캔 확정 요청 프로토콜 생성
 */
public class WareSerialScanConfirmProtocol {
    private SerialScanInfo mInfo = null;

    public WareSerialScanConfirmProtocol(SerialScanInfo info) {
        mInfo = info;
    }

    /**
     * 입고 시리얼 스캔 확정 요청을 처리하는 WAS 메소드 반환
     * @return 입고 시리얼 스캔 확정 요청을 처리하는 WAS 메소드
     */
    public String getReqMethod() {
        return "/ctrl/inbound/inboundSerialPda/insertIbSerialPda";
    }

    /**
     * 입고 시리얼 스캔 확정 요청 내용 반환
     * @return JSON 형식의 문자열.
     */
