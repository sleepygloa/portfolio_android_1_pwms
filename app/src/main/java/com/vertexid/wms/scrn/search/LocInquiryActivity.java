...
import com.vertexid.wms.scrn.search.adapter.LocInquiryItemAdapter;
 ...
 * 위치 조회 화면
 */
public class LocInquiryActivity extends BaseActivity {
    ...
    private final int HANDLER_SHOW_SCAN_RESULT = HANDLER_SHOW_LIST + 1;
    ...
    private LocInquiryItemAdapter mAdapter = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.loc_inquiry_activity);
        ...
        mDrawerLayout = (DrawerLayout) findViewById(R.id.loc_inquiry_drawer);
        NavigationView navi_view = (NavigationView) findViewById(R.id.loc_inquiry_navi_view);
        ...
        TextView text_user = (TextView) findViewById(R.id.loc_inquiry_text_user);
        text_user.setText(user_id + "(" + center.getText() + "/" + customer.getText() + ")");

        mEditLoc = (EditText) findViewById(R.id.loc_inquiry_edit_loc);
        mEditLotId = (EditText) findViewById(R.id.loc_inquiry_edit_lot_id);
        mEditMakeDate = (EditText) findViewById(R.id.loc_inquiry_edit_manufacture_date);

        mEditMakeDate.setOnClickListener(mClickListener);
        mEditMakeDate.setInputType(InputType.TYPE_NULL);

        mEditLoc.setOnEditorActionListener(mEditorActionListener);
        mEditLotId.setOnEditorActionListener(mEditorActionListener);

        mEditLoc.requestFocus();

        FrameLayout layout_title_menu = (FrameLayout) findViewById(R.id.loc_inquiry_layout_btn_menu);
        layout_title_menu.setOnClickListener(mClickListener);
        Button btn_inquiry = (Button) findViewById(R.id.loc_inquiry_btn_inquiry);
        ...
        mAdapter = new LocInquiryItemAdapter(mContext, mArray);
        ListView list_view = (ListView) findViewById(R.id.loc_inquiry_listview);
    ...
    private void showScanResult(String scan) {
        ...
        if (mEditLoc.isFocused()) {
            mEditLoc.setText(scan);
            mEditLotId.requestFocus();
            imm.hideSoftInputFromWindow(mEditLoc.getWindowToken(), 0);
        }
    ...
    private void showInfo(int position) {
        ...
        EditText edit_box = (EditText) findViewById(R.id.loc_inquiry_edit_acquire_box);
        edit_box.setText(String.valueOf(box));
        EditText edit_ea = (EditText) findViewById(R.id.loc_inquiry_edit_acquire_ea);
...
//                Object obj4 = VUtil.getValue(json_obj, NetworkParam.MAKE_DATE);
//                String make_date = "";
//                if (obj4 != null && !obj4.toString().equals("null")) {
//                    make_date = (String) obj4;
//                }
    ...
    private void reqInquiry() {
        ...
        if (loc == null || loc.length() <= 0) {
            String text = getString(R.string.not_exist_loc);
            showPopup(text);
            return ;
        }
        ...
        network.reqGoodsLocInquiry(info);
    }
    ...
    private View.OnClickListener mClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            int id = v.getId();
            if (id == R.id.loc_inquiry_layout_btn_menu) {
                mDrawerLayout.openDrawer(GravityCompat.START);
            }
            else if (id == R.id.loc_inquiry_edit_manufacture_date) {
                showCalendar();
            }
            else if (id == R.id.loc_inquiry_btn_inquiry) {
                reqInquiry();
            }
        }
    };
