 ...
 * 출고피킹 화면
 */
public class TakeOutPickingActivity extends BaseActivity {
    ...
    private EditText mEditPickingLocScan = null;    //피킹LOC 스캔
    private EditText mEditPltIdScan = null;
    private EditText mEditPickingBox = null;        //피킹수량 BOX
    private EditText mEditPickingEa = null;         //피킹수량 EA
    private EditText mEditReason = null;            //미출고 사유
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.take_out_picking_activity);
        ...
        if (intent != null) {
            mInfo = (TakeOutInfo) intent.getSerializableExtra(ScrnParam.INFO_DETAIL);
            mPosition = intent.getIntExtra(ScrnParam.INFO_POSITION, 0);
        }

        mDrawerLayout = (DrawerLayout) findViewById(R.id.take_out_picking_drawer);
        NavigationView navi_view = (NavigationView) findViewById(R.id.take_out_picking_navi_view);
        ...
        TextView text_user = (TextView) findViewById(R.id.take_out_picking_text_user);
        text_user.setText(user_id + "(" + center.getText() + "/" + customer.getText() + ")");

        Button btn_menu = (Button) findViewById(R.id.take_out_picking_btn_menu);
        btn_menu.setOnClickListener(mClickListener);
        Button btn_confirm = (Button) findViewById(R.id.take_out_picking_btn_confirm);
        btn_confirm.setOnClickListener(mClickListener);
        ImageButton btn_reason = (ImageButton) findViewById(R.id.take_out_picking_btn_not_take_out_reason);
        btn_reason.setOnClickListener(mClickListener);

        EditText edit_picking_loc = (EditText) findViewById(R.id.take_out_picking_edit_picking_loc);
        mEditPickingLocScan = (EditText) findViewById(R.id.take_out_picking_edit_picking_loc_scan);
        EditText edit_goods_code = (EditText) findViewById(R.id.take_out_picking_edit_goods_code);
        EditText edit_goods_name = (EditText) findViewById(R.id.take_out_picking_edit_goods_name);
        EditText edit_plt_id = (EditText) findViewById(R.id.take_out_picking_edit_plt_id);
        mEditPltIdScan = (EditText) findViewById(R.id.take_out_picking_edit_plt_id_scan);
        EditText edit_manufacture_lot = (EditText) findViewById(R.id.take_out_picking_edit_manufacture_lot);
        EditText edit_make_date = (EditText) findViewById(R.id.take_out_picking_edit_manufacture_date);
        EditText edit_end_date = (EditText) findViewById(R.id.take_out_picking_edit_distribution_date);
        EditText edit_lot1 = (EditText) findViewById(R.id.take_out_picking_edit_lot1);
        EditText edit_lot2 = (EditText) findViewById(R.id.take_out_picking_edit_lot2);
        EditText edit_lot3 = (EditText) findViewById(R.id.take_out_picking_edit_lot3);
        EditText edit_lot4 = (EditText) findViewById(R.id.take_out_picking_edit_lot4);
        EditText edit_lot5 = (EditText) findViewById(R.id.take_out_picking_edit_lot5);
        EditText edit_order_box = (EditText) findViewById(R.id.take_out_picking_edit_direction_box);
        EditText edit_order_ea = (EditText) findViewById(R.id.take_out_picking_edit_direction_ea);
        mEditPickingBox = (EditText) findViewById(R.id.take_out_picking_edit_picking_box);
        mEditPickingEa = (EditText) findViewById(R.id.take_out_picking_edit_picking_ea);
        mEditReason = (EditText) findViewById(R.id.take_out_picking_edit_reason);

        mEditPickingLocScan.requestFocus();

        edit_picking_loc.setText(mInfo.getPickingLoc());
        edit_goods_code.setText(mInfo.getGoods());
        edit_goods_name.setText(mInfo.getGoodsName());
        edit_plt_id.setText(mInfo.getPalletID());
        edit_manufacture_lot.setText(mInfo.getManufactureLot());
        edit_make_date.setText(mInfo.getMakeDate());
        edit_end_date.setText(mInfo.getDistributionDate());
        edit_lot1.setText(mInfo.getLotAttribute1());
        edit_lot2.setText(mInfo.getLotAttribute2());
        edit_lot3.setText(mInfo.getLotAttribute3());
        edit_lot4.setText(mInfo.getLotAttribute4());
        edit_lot5.setText(mInfo.getLotAttribute5());
        edit_order_box.setText(String.valueOf(mInfo.getDirectionBox()));
        edit_order_ea.setText(String.valueOf(mInfo.getDirectionEa()));
        mEditPickingBox.setText(String.valueOf(mInfo.getPickingBox()));
        mEditPickingEa.setText(String.valueOf(mInfo.getPickingEa()));
    }
     ...
     * 미출고사유를 리스트 형식으로 보여줌
     */
    private void showNotTakeOutReason() {
        ...
        IConfigManager config = new ConfigManager(mContext);
        mArrayReason = config.getCodeList(CodeInfo.CODE_TYPE_NOT_TAKE_OUT_REASON);
        if (mArrayReason == null) {
            return ;
        }
        ...
        builder.setTitle("미출고 사유를 선택해주세요.");
    ...
    private void showScanResult(String text) {
        ...
        if (mEditPickingLocScan.isFocused()) {
            mEditPickingLocScan.setText(text);
            mEditPltIdScan.requestFocus();
            imm.hideSoftInputFromWindow(mEditPickingLocScan.getWindowToken(), 0);
        }
    ...
    private void showConfirmResult(JSONObject json) {
        ...
        builder.setPositiveButton("확인", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialogInterface, int i) {
                mInfo.setProcessState(TakeOutInfo.CONFIRM_COMPLETE);
                moveNextActivity();
            }
        });
        builder.show();
    ...
    private void reqConfirm() {
        ...
        String loc_scan = mEditPickingLocScan.getText().toString();
        if (loc_scan == null || loc_scan.length() == 0) {
            showPopup(getString(R.string.not_exist_picking_loc_scan));
            return ;
        }

        String plt_id_scan = mEditPltIdScan.getText().toString();
        if (plt_id_scan == null || plt_id_scan.length() == 0) {
            showPopup(getString(R.string.not_exist_picking_plt_id_scan));
            return ;
        }

        String edit_box_count = mEditPickingBox.getText().toString();
        int box_count = 0;
        if (edit_box_count != null && edit_box_count.length() > 0) {
            box_count = Integer.parseInt(edit_box_count);
        }

        String edit_ea_count = mEditPickingEa.getText().toString();
        int ea_count = 0;
        if (edit_ea_count != null && edit_ea_count.length() > 0) {
            ea_count = Integer.parseInt(edit_ea_count);
        }
        ...
        if (order_box != null && order_box.length() > 0) {
            order_box_count = Integer.parseInt(order_box);
        }

        int order_ea_count = 0;
        if (order_ea != null && order_ea.length() > 0) {
            order_ea_count = Integer.parseInt(order_ea);
        }
        ...
        String reason_code = "";
        // box 또는 ea 중 어느하나라도 지시수량보다 미달이면
        // 미출고 사유를 선택하라는 팝업을 보여줌
        if (order_box_count > box_count || order_ea_count > ea_count) {
            if (reason.length() <= 0) {
                showPopup(getString(R.string.popup_show_not_take_out_reason));
                return ;
            }else {
                for (int i = 0 ; i < mArrayReason.size() ; i++) {
        ...
        mInfo.setPickingBox(box_count);
        mInfo.setPickingEa(ea_count);
        mInfo.setReason(reason_code);
        ...
        network.reqTakeOutPickingListConfirm(mInfo);
    }
    ...
    private View.OnClickListener mClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            int id = v.getId();
            if (id == R.id.take_out_picking_btn_menu) {
                mDrawerLayout.openDrawer(GravityCompat.START);
            }
            else if (id == R.id.take_out_picking_btn_not_take_out_reason) {
                showNotTakeOutReason();
            }
            else if (id == R.id.take_out_picking_btn_confirm) {
                reqConfirm();
            }
        }
    };
