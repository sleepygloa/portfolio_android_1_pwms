 ...
 * 피킹(총량) 출고피킹화면
 */
public class TakeOutPickingTotalDetailActivity extends BaseActivity {
    ...
    private EditText mEditLoc = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.take_out_picking_total_detail_activity);
        ...
        mDrawerLayout = (DrawerLayout) findViewById(R.id.take_out_picking_total_detail_drawer);
        NavigationView navi_view = (NavigationView) findViewById(R.id.take_out_picking_total_detail_navi_view);
        ...
        TextView text_user = (TextView) findViewById(R.id.take_out_picking_total_detail_text_user);
        text_user.setText(user_id + "(" + center.getText() + "/" + customer.getText() + ")");

        mEditLocScan = (EditText) findViewById(R.id.take_out_picking_total_detail_edit_loc_scan);
        mEditPltIdScan = (EditText) findViewById(R.id.take_out_picking_total_detail_edit_plt_id_scan);
        mEditPickingBox = (EditText) findViewById(R.id.take_out_picking_total_detail_edit_picking_box);
        mEditPickingEa = (EditText) findViewById(R.id.take_out_picking_total_detail_edit_picking_ea);
        mEditReason = (EditText) findViewById(R.id.take_out_picking_total_detail_edit_reason);
        ...
        mEditLoc = (EditText) findViewById(R.id.take_out_picking_total_detail_edit_loc);
        EditText edit_goods_code = (EditText) findViewById(R.id.take_out_picking_total_detail_edit_goods_code);
        EditText edit_goods_name = (EditText) findViewById(R.id.take_out_picking_total_detail_edit_goods_name);
        mEditPltId = (EditText) findViewById(R.id.take_out_picking_total_detail_edit_plt_id);
        EditText edit_order_box = (EditText) findViewById(R.id.take_out_picking_total_detail_edit_order_box);
        EditText edit_order_ea = (EditText) findViewById(R.id.take_out_picking_total_detail_edit_order_ea);

        mEditLoc.setText(mInfo.getPickingLoc());
        edit_goods_code.setText(mInfo.getGoods());
        edit_goods_name.setText(mInfo.getGoodsName());
        mEditPltId.setText(mInfo.getPalletID());
        edit_order_box.setText(String.valueOf(mInfo.getDirectionBox()));
        edit_order_ea.setText(String.valueOf(mInfo.getDirectionEa()));

        Button btn_menu = (Button) findViewById(R.id.take_out_picking_total_detail_btn_menu);
        btn_menu.setOnClickListener(mClickListener);
        Button btn_reason = (Button) findViewById(R.id.take_out_picking_total_detail_btn_reason);
        btn_reason.setOnClickListener(mClickListener);
        Button btn_confirm = (Button) findViewById(R.id.take_out_picking_total_detail_btn_confirm);
    ...
    private void showNotTakeOutReason() {
        ...
        String [] reasons = new String[mArrayReason.size()];
        for (int i = 0 ; i < mArrayReason.size() ; i++) {
            reasons[i] = mArrayReason.get(i).getText();
        }
    ...
    private void reqConfirm() {
        String edit_box = mEditPickingBox.getText().toString();
        int box_count = 0;
        if (edit_box != null && edit_box.length() > 0) {
            box_count = Integer.parseInt(edit_box);
        }

        String edit_ea = mEditPickingEa.getText().toString();
        int ea_count = 0;
        if (edit_ea != null && edit_ea.length() > 0) {
            ea_count = Integer.parseInt(edit_ea);
        }
        ...
        String reason_text = mEditReason.getText().toString();
        ...
        String loc = mEditLoc.getText().toString();
        if (loc != null && !loc.equals(loc_scan)) {
            showPopup(getString(R.string.not_equal_picking_loc));
            return ;
        }

        if (!loc.equals(loc_scan)) {
            showPopup(getString(R.string.not_equal_picking_loc));
            return ;
        }
        ...
        if (plt_id != null && !plt_id.equals(plt_id_scan)) {
        ...
        network.reqTakeOutPickingTotalConfirm(mInfo);
    }
    ...
    private View.OnClickListener mClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            int id = v.getId();
            if (id == R.id.take_out_picking_total_detail_btn_menu) {
                mDrawerLayout.openDrawer(GravityCompat.START);
            }
            else if (id == R.id.take_out_picking_total_detail_btn_reason) {
                showNotTakeOutReason();
            }
            else if (id == R.id.take_out_picking_total_detail_btn_confirm) {
                reqConfirm();
            }
        }
    };
